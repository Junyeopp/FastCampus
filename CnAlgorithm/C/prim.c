...
int main(void) {
    ...
    for (int i = 0; i < m; i++) {
        int a, b, c;
        printf("a b c : ");
        scanf("%d %d %d", &a, &b, &c);  // a에서 b까지 c만큼 소요
    ...
    for (int i = 1; i <= n; i++) {
        ...
        if (nextCost == INT_MAX) printf("disconnected graph\n");
        res += nextCost;
        d[nextNode] = 1;
